...
def create_dataframe(data, include_kernel_filter, train, test, folder):
    ...
    if not include_kernel_filter:
        # Drop rows where index (the first column in transposed df) contains 'kernel_filter'
        df = df[~df.index.str.contains('kernel_')]
...
def main():
    ...
    all_dfs_aa = []
    all_dfs_ss8 = []
    include_kernel_filter = False  # Set to True / False to include/exclude kernel_filter  这里可决定是否在 DataFrame 中包含 kernel_filter 数据，还是仅包含 score数据

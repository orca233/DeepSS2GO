...
def main():
    ...
    organisms = ['ARATH', 'ECOLI', 'HUMAN', 'MOUSE', 'MYCTU', 'YEAST']
    all_dfs = []

    for folder in os.listdir(base_path):
        if re.match(r'test_Train\w+_Test\w+_(aa|ss8)', folder):
            print('11111 folder = ', folder)
            ...
            if os.path.exists(folder_path):
                ...
                data = {
                    ...
                    'kernel_filter': [results['bp']['kernel_fmax'], results['bp']['kernel_aupr'],
                                      results['bp']['kernel_smin'],
                                      results['cc']['kernel_fmax'], results['cc']['kernel_aupr'],
                                      results['cc']['kernel_smin'],
                                      results['mf']['kernel_fmax'], results['mf']['kernel_aupr'],
                                      results['mf']['kernel_smin']]
                }

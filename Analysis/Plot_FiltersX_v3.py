...
for i, index in enumerate(column_indices):  # i 代表小图的索引
    ...
    # new 20231226，使得每个小图中的legand只显示kernel和数字。 比如之前是: kernel16_cc_AUPR，需要改成kernel16。

    # 取包含"bp_Fmax/cc_AUPR/mf_Smin"等的列，提取数据并作小图
    marker_index = 0
    for j in range(1, data.shape[1]):
        if column_names[i] in data.columns[j]:  # column_names[i]='bp_Fmax/cc_AUPR...'。 data.columns[j]是csv文件中第j列名字, eg:kernel8_cc_Smin 或 eg:kernel8
            y = data.iloc[:, j]
            # 修改这里：提取并更改图例标签
            title_parts = data.columns[j].split('_')
            legend_label = f'kernel{title_parts[0][6:]}'  # 例如："kernel16" 或 "kernel128"
            marker = markers[marker_index]  # 按顺序选择一个标记  markers[j % len(markers)]
            marker_index += 1
            ax.plot(x, y, label=legend_label, linewidth=2, marker=marker, markersize=5)  # linestyle='-'

            # 添加图例
            # if i == 6 or i == 7 or i == 8:  # bp/cc/mf_smin 这个图，画legend
            # if 3 <= i and i <= 8:  # bp/cc/mf_smin 这个图，画legend
            ax.legend(fontsize=10, loc='lower left', ncol=2)  # 3列
    ...
    # new 20231226，
    # 使得在小图内部添加文本显示最大值和最小值时，如果小图标题包含Fmax或AUPR，则只显示最大值。如果小图标题包含Smin，则只显示最小值
    # 获取小图的标题
    subplot_title = column_names[i]

    # 在小图内部添加文本显示最大值和最小值
    if 'Fmax' in subplot_title or 'AUPR' in subplot_title:
        # 如果标题包含Fmax或AUPR，只显示最大值
        ax.text(0.05, 0.95, f"Max: {max_value:.3f} (x={max_x:.1f})", fontsize=10, ha='left', va='top',
                transform=ax.transAxes)
    elif 'Smin' in subplot_title:
        # 如果标题包含Smin，只显示最小值
        ax.text(0.05, 0.95, f"Min: {min_value:.3f} (x={min_x:.1f})", fontsize=10, ha='left', va='top',
                transform=ax.transAxes)

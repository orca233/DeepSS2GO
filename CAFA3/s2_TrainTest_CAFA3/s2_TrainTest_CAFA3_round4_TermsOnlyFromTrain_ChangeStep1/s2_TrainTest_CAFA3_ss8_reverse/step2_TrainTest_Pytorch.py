...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'data/model_checkpoint.pth')  # 存迄今最优模型参数， model_file='data/model_checkpoint.pth'
...
class ProteinGODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seq = row.sequences
        onehot = to_onehot(seq)  # 将蛋白质序列转换为one-hot编码   # onehot:  'numpy.ndarray, (2000,21)
        data_onehot = torch.zeros((params_local['MAXLEN'], PROT_LETTER_len+1), dtype=torch.float32)  # 初始化一个全零张量，用于存储one-hot编码后的序列，original:((MAXLEN, 21)
        labels = torch.zeros(terms_classes, dtype=torch.float)  # 初始化一个全零张量，用于存储标签  original:dtype=torch.int32 or torch.long
        data_onehot[:onehot.shape[0], :] = torch.from_numpy(onehot)  # 将one-hot编码复制到data_onehot张量中，保证长度一致。 onehot.shape[0]=MAXLEN对不？
...
class Model1(nn.Module):
    def __init__(self, terms_classes, params):
...
def run_train(model, train_dataloader, valid_dataloader):
    ################## 训练部分 ###############
    # 定义Early Stopping
...
PROT_LETTER = []
PROT_LETTER_len = -1
if params_local['aa_ss'] == 'aa':
    PROT_LETTER = params_local['PROT_LETTER_aa']
    PROT_LETTER_len = len(PROT_LETTER)  # =20
elif params_local['aa_ss'] == 'ss8':
    PROT_LETTER = params_local['PROT_LETTER_ss8']
    PROT_LETTER_len = len(PROT_LETTER)  # =8
elif params_local['aa_ss'] == 'ss3':
    PROT_LETTER = params_local['PROT_LETTER_ss3']
...
model_file = 'data/model_checkpoint.pth'  # ???
predictions_file = 'data/predictions.pkl'
logger_file = 'data/training.csv'  # 每一步epoch保存一个loss

# e.g. terms_file = 'data/terms_mf.pkl'  或者  'data/terms_all.pkl'
...
train_dataset = ProteinGODataset(train_df, terms_dict)
valid_dataset = ProteinGODataset(valid_df, terms_dict)
test_dataset = ProteinGODataset(test_df, terms_dict)
...
if params_local['load_pretrained_model'] == 0:   # 不加载预训练的模型，则需要train
    run_train(model, train_dataloader, valid_dataloader)  # 可以控制是否跳过train阶段
...
# 加载已训练的模型
# model = Model1(terms_classes, params_local)  # -- 前面加载过了


# ##### 下面这几行注释掉的内容，是打算分开计算 Train & Test
# # 如果不进行训练 run_step2_Train = F，则要加载预训练的模型。
# # 如果进行训练 run_step2_Train = T， model_file选取默认，也就是上面train出来的  model_file = 'data/model_checkpoint.pth'
# if params_local['run_step2_Train'] == 'F':
#     load_pretrained_model_root_path = params_local['load_pretrained_model_root_path']  # test_TrainHUMAN_TestARATH_aa/step3_done/  其实这是dir0的延伸
#
#     # 因为这个step3会在local子目录中运行，所以要获得当前文件夹名称，和root_base一起组成完整的pretrained_model路径
#     current_directory = os.getcwd()  # 当前 绝对路径，
#     folder_name = os.path.basename(current_directory)  # 当前文件夹名称
#     # /home/fsong/work/py_proj/prot_algo/DeepSS2GO_Pytorch/  (path_base)
#     # output/test_TrainHUMAN_TestARATH_aa/step3_done/  (load_pretrained_model_root_path)
#     # DeepSS2GO_Kernel8_Filter8192_Ontsall  (folder_name)
#     model_file = params_local['path_base'] + load_pretrained_model_root_path + folder_name + '/data/model_checkpoint.pth'
...
# 测试模型并保存预测结果
run_test(model, test_dataloader)
